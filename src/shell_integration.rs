...
fn create_fish_function_content() -> String {
    let (shell_name, function_name, binary_logic, usage_info) = (
...
fn create_posix_shell_function_content() -> String {
    let (shell_name, function_name, binary_logic, alias_section) = (
...
async fn setup_zsh_integration() -> Result<()> {
    ...
    let zsh_content = create_posix_shell_function_content();
...
async fn setup_bash_integration() -> Result<()> {
    ...
    let bash_content = create_posix_shell_function_content();
...
async fn add_dev_env_to_zsh(binary_path: &Path) -> Result<()> {
    ...
    let dev_env_section = format!(
        r#"# Development environment - auto-configured by pm init
export _PM_BINARY="{}"
